...
internal void
drawRect_in_pixels(int x0, int y0, int x1, int y1, u32 color) {
...
drawRect(float x, float y, float half_size_x, float half_size_y, u32 color) {
	...
	drawRect_in_pixels(x0, y0, x1, y1, color);
}


internal void
drawNumber(int num, float x, float y, float size, u32 color) {
...
internal void
drawText(const char* text, float x, float y, float size, u32 color) {
	float half_size = size * .5f;
	float original_y = y;
